...
public class Calendar {
    ...
    public Calendar(String arg_year) {

        date = LocalDate.of(Integer.parseInt(arg_year), LocalDate.now().getMonth().getValue(), LocalDate.now().getDayOfMonth());
        Calendar_create();
        Calendar_create();
    }

    public Calendar(String arg_year, String arg_month) {
        date = LocalDate.of(Integer.parseInt(arg_year), Integer.parseInt(arg_month), LocalDate.now().getDayOfMonth());
        Calendar_create();
    }
/*
    public Calendar(String arg_year, String arg_month, String arg_type) {
        date = LocalDate.of(Integer.parseInt(arg_year), Integer.parseInt(arg_month), LocalDate.now().getDayOfMonth());
...
*/
    ...
    public void printCalendarHTML() {//выводим календарик
        ...
        table.append("<tr>\n");
        for (int i = 1; i <= 7; i++) {//сначала названия дней недели
            table.append("<td>");
            table.append(DayOfWeek.of(i).getDisplayName(TextStyle.SHORT, Locale.ENGLISH));
            table.append("</td>\n");
        }
        ...
        for (int i = 0; i < 6; i++) {
            table.append("<tr>\n");
            for (int j = 0; j < 7; j++) {//сначала названия дней недели

                if (month[i][j].getDay() != 0) {
                    table.append("<td>");
                    table.append(month[i][j].getDay());
                    table.append("</td>\n");
                }
                else{
                    table.append("<td>");
